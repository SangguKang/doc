...
verlt `uname -r` 3.2.0
if [ $? = 0 ]; then
	echo "Error: The SDK needs a kernel > 3.2.0"
	exit 1
fi
...
SDK_BUILD_PATH="/opt/yoda/24.1.1"
DEFAULT_INSTALL_DIR="/opt/yoda/24.1.1"
SUDO_EXEC=""
EXTRA_TAR_OPTIONS=""
target_sdk_dir=""
answer=""
...
titlestr="Yoda (VIAVI Instrument Platform Distro) SDK installer version 24.1.1"
printf "%s\n" "$titlestr"
printf "%${#titlestr}s\n" | tr " " "="
...
if [ -e "$target_sdk_dir/environment-setup-aarch64-poky-linux" ]; then
...
for env_setup_script in `ls $target_sdk_dir/environment-setup-*`; do
	...
	$SUDO_EXEC sed -e "s:/opt/yoda/24.1.1:$target_sdk_dir:g" -i $env_setup_script
done
...
# replace /opt/yoda/24.1.1 with the new prefix in all text files: configs/scripts/etc.
...
# change all symlinks pointing to /opt/yoda/24.1.1
...
if [ -e "$post_relocate" ]; then
	$SUDO_EXEC sed -e "s:/opt/yoda/24.1.1:$target_sdk_dir:g" -i $post_relocate
	$SUDO_EXEC /bin/sh $post_relocate "$target_sdk_dir" "/opt/yoda/24.1.1"
...
MARKER:
?zXZ  煉퀳핞댼[`!   E얨?x靖] ?}빥J>y쫄&?똈꺜?쩘癸?귁%?棠r=?er/텐뭟+柔薰칾첝l붼的굃%?쟪叛G' 禎m6?촟7R??탡臾驀?p}m?갖9餌?>?y?E
)팷HKu쉫i?:<?ILZd?燒q?쮼i胛?췴e稈b0
묋	?줫f?u쀧/#}q섌石욦뽒탵X?Jp??컕?4굃?
이하...
